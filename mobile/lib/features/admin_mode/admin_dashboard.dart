...
class _AdminDashboardState extends ConsumerState<AdminDashboard> with SingleTickerProviderStateMixin {
  ...
  Color _urgencyColor(String urgency) {
    switch (urgency) {
      ...
      case 'urgent': return const Color(0xFFFF9100);
  ...
  Color _issueTypeColor(String type) {
    switch (type) {
      case 'medical_emergency': return const Color(0xFFFF1744);
      case 'legal_aid': return const Color(0xFF448AFF);
      case 'disaster_relief': return const Color(0xFFFF6D00);
  ...
  Widget _buildIssuesTab(ThemeData theme, bool isDark) {
    if (_reports.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.check_circle_outline, size: 64, color: theme.colorScheme.outlineVariant),
    ...
    final critical = _reports.where((r) => r['user_urgency'] == 'critical').length;
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              _StatChip(label: 'Total', value: '${_reports.length}', color: const Color(0xFF448AFF)),
              _StatChip(label: 'Critical', value: '$critical', color: const Color(0xFFFF1744)),
              _StatChip(label: 'Urgent', value: '$urgent', color: const Color(0xFFFF9100)),
              _StatChip(label: 'Pending', value: '$pending', color: Colors.grey),
            ],
          ),
        ),
        Expanded(
          child: RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final issueType = (report['issue_type'] as String?) ?? 'civic_issue';
                final status = (report['status'] as String?) ?? 'pending';
                final reqVol = (report['required_volunteers'] as num?)?.toInt() ?? 1;
                final assigned = (report['assigned_volunteer_ids'] as List?)?.length ?? 0;
                          ...
                          children: [
                            Container(
                              ...
                              child: Center(child: Text('#${index + 1}',
                                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: _urgencyColor(urgency)))),
                            ),
                            const SizedBox(width: 8),
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                              decoration: BoxDecoration(
                                color: _issueTypeColor(issueType).withOpacity(0.12),
                                borderRadius: BorderRadius.circular(6),
                              ),
                              child: Text(issueType.replaceAll('_', ' '),
                                  style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: _issueTypeColor(issueType))),
                            ),
                            ...
                            Container(
                              ...
                              child: Text(urgency.toUpperCase(),
                                  style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: _urgencyColor(urgency))),
                            ),
                            const Spacer(),
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                              decoration: BoxDecoration(
                                color: status == 'resolved' ? Colors.green.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
                                borderRadius: BorderRadius.circular(6),
                              ),
                              child: Text(status.replaceAll('_', ' '),
                                  style: theme.textTheme.labelSmall?.copyWith(
                                    fontWeight: FontWeight.w600,
                                    color: status == 'resolved' ? Colors.green : theme.colorScheme.onSurfaceVariant,
                                  )),
                            ),
                          ],
                        ),
                        const SizedBox(height: 10),
                        Text((report['raw_text'] as String?) ?? 'No description',
                            style: theme.textTheme.bodyMedium, maxLines: 2, overflow: TextOverflow.ellipsis),
                        const SizedBox(height: 8),
                        Row(
                          children: [
                            if ((report['location'] as String?)?.isNotEmpty == true) ...[
                              Icon(Icons.location_on, size: 14, color: theme.colorScheme.onSurfaceVariant),
                              const SizedBox(width: 2),
                              Expanded(child: Text(report['location'] as String? ?? '',
                            ...
                            const SizedBox(width: 2),
                            Text('$assigned/$reqVol assigned', style: theme.textTheme.labelSmall),
                          ],
                        ),
                        const SizedBox(height: 10),
                        SizedBox(
                          ...
                          child: FilledButton.icon(
                            onPressed: () => _assignVolunteers(report['id'] as String),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.people_alt_rounded, size: 64, color: Color(0xFF6C63FF)),
            const SizedBox(height: 16),
            Text('Volunteer Matching', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            Text('Select an issue from the Issues tab and tap "Find & Assign" to match volunteers.',
...
class _HeatmapTabState extends State<_HeatmapTab> {
  ...
  LatLng _computeCenter() {
    if (widget.reports.isEmpty) return const LatLng(26.8467, 80.9462); // Default: Lucknow
    ...
    if (count == 0) return const LatLng(26.8467, 80.9462);
  ...
  Set<Marker> _buildMarkers() {
    ...
    for (int i = 0; i < widget.reports.length; i++) {
      ...
      final snippet = rawText.length > 60 ? '${rawText.substring(0, 60)}...' : rawText;
  ...
  Set<Circle> _buildHeatCircles() {
    ...
    for (int i = 0; i < widget.reports.length; i++) {
      ...
      final urgency = (r['user_urgency'] as String?) ?? 'normal';
      final radius = urgency == 'critical' ? 500.0 : (urgency == 'urgent' ? 350.0 : 200.0);
      ...
      circles.add(Circle(
        ...
        radius: radius,
        fillColor: fillColor,
        strokeColor: fillColor.withOpacity(0.5),
        strokeWidth: 1,
      ));
    }
    return circles;
  ...
  Widget build(BuildContext context) {
    ...
    final criticalCount = widget.reports.where((r) => r['user_urgency'] == 'critical').length;
    final urgentCount = widget.reports.where((r) => r['user_urgency'] == 'urgent').length;
    final pendingCount = widget.reports.where((r) => r['status'] == 'pending').length;
          ...
          child: Card(
            ...
            child: Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  _LegendItem(color: const Color(0xFFFF1744), label: 'Critical ($criticalCount)'),
                  _LegendItem(color: const Color(0xFFFF9100), label: 'Urgent ($urgentCount)'),
                  _LegendItem(color: const Color(0xFF00E676), label: 'Normal'),
                  Text('$totalReports total', style: theme.textTheme.labelSmall?.copyWith(fontWeight: FontWeight.bold)),
                ],
              ),
            ),
          ),
          ...
          child: Column(
            children: [
              ...
              _MapButton(
                ...
                onTap: () => _mapController?.animateCamera(CameraUpdate.newLatLngZoom(center, 12)),
              ),
              const SizedBox(height: 8),
              _MapButton(
                icon: Icons.zoom_in,
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceAround,
              children: [
                _BottomStat(icon: Icons.warning_amber_rounded, value: '$totalReports', label: 'Issues', color: const Color(0xFF448AFF)),
                _BottomStat(icon: Icons.error_outline, value: '$criticalCount', label: 'Critical', color: const Color(0xFFFF1744)),
                _BottomStat(icon: Icons.schedule, value: '$pendingCount', label: 'Pending', color: const Color(0xFFFF9100)),
                _BottomStat(icon: Icons.check_circle_outline, value: '${totalReports - pendingCount}', label: 'Active', color: const Color(0xFF00E676)),
              ],
            ),
          ),
        ),
...
class _BottomStat extends StatelessWidget {
  ...
  final String label;
  final Color color;
  const _BottomStat({required this.icon, required this.value, required this.label, required this.color});
...
class _VolunteerMatchSheet extends StatefulWidget {
  ...
  const _VolunteerMatchSheet({required this.reportId, required this.api, required this.onAssigned});
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text('Matching Volunteers', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
                  const Spacer(),
                  FilledButton.icon(
                  ...
                  : _matches.isEmpty
                      ? Center(
                          child: Column(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(Icons.person_search, size: 48, color: theme.colorScheme.outlineVariant),
                              const SizedBox(height: 12),
                              Text('No matching volunteers found', style: theme.textTheme.bodyLarge),
                              const SizedBox(height: 4),
                              Text('Volunteers will appear when they sign up', style: theme.textTheme.bodySmall),
                            ],
                          ),
                        )
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final score = (m['total_score'] as num?)?.toStringAsFixed(1) ?? '0';
                            ...
                            return CheckboxListTile(
                              ...
                              title: Text(m['volunteer_name'] as String? ?? 'Volunteer',
