...
class _DigitizationDashboardState extends State<DigitizationDashboard> {
  final List<_OcrItem> _queue = [
    _OcrItem(id: '1', title: 'Survey Form — Ward 14', pages: 3, status: 'pending'),
    _OcrItem(id: '2', title: 'Census Record — Block C', pages: 7, status: 'processing'),
    _OcrItem(id: '3', title: 'Land Revenue — Khasra 45', pages: 2, status: 'completed'),
    _OcrItem(id: '4', title: 'Ration Card App — Batch 12', pages: 4, status: 'pending'),
    _OcrItem(id: '5', title: 'Birth Certificate — Jan 2025', pages: 1, status: 'pending'),
  ];

  int get _pendingCount => _queue.where((i) => i.status == 'pending').length;
  int get _completedCount => _queue.where((i) => i.status == 'completed').length;
      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                _StatCard(
                  label: 'Processing',
                  value: '${_queue.where((i) => i.status == 'processing').length}',
                  color: Colors.blue,
          ...
          Padding(
            ...
            child: Row(
              children: [
                Text('OCR Queue', style: theme.textTheme.titleMedium?.copyWith(
  ...
  IconData _statusIcon(String status) {
    switch (status) {
      ...
      case 'processing': return Icons.document_scanner;
...
class _OcrItem {
  ...
  _OcrItem({required this.id, required this.title, required this.pages, required this.status});
}
